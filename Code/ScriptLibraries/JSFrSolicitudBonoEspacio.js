...
function camposValidos(){
	var aCampos = ["jefe","jornada","fechaRedencion"];
	...
	if(!application.bEsAdmin){
		if (ordenFechasValido($("[name='fechaRedencion']").val(), "<", documento.sFechaActual)){
			bAlert("La fecha de redención debe ser mayor al día actual.");
			return false;
		}		
	}
	return true;
...
function validarSolicitud(x){
		var jornada;
		var url = application.sWebDbName + "agValidarSolicitudMes?Open&autor=" + documento.sAutorAbbreviate + "&fechaRedencion=" +(application.bEdicion ?$("[name = fechaRedencion]").val() :documento.sFechaRedencion) +"&jornada="+(application.bEdicion ?$("[name = jornada]").val(): documento.sJornada)+"&unique="+application.sUnique+"&cargo="+(application.bEdicion ?$("[name = cargo]").val() :documento.sCargo)+"&solicitud=frConfBonoEspacio&form="+application.sForm;
