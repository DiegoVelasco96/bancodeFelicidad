		...
		$.getJSON(url, function(data){			
			if (data.msgError == ""){
				...
				if (data.jornadaRepetida > 0 ){
					...
					bAlert(documento.sAutorCommon+" ha registrado en el sistema una solicitud con el mismo horario ("+jornada+").  No es posible generar una nueva solicitud con la misma información");
					return false;
				}
				
				
				if (data.anio > 0){
					bAlert("La solicitud excede el número de horarios disponibles para el año "+data.anioActual+", no es posible generar una nueva solicitud para este año");
					return false;
				}
				if(x == "g"){
