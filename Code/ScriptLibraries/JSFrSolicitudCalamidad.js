		...
		$.getJSON(url, function(data){			
			if (data.msgError == ""){
				...
				if (data.jornadaRepetida > 0 ){
					...
					bAlert(documento.sAutorCommon+" ha registrado en el sistema una solicitud con el mismo número de días ("+jornada+").  No es posible generar una nueva solicitud con la misma información");
					return false;
				}
				
				
				if (data.anio > 0){
					bAlert("La solicitud excede el número de días disponibles para el año "+data.anioActual+", no es posible generar una nueva solicitud para este año");
					return false;
				}
				if(x == "g"){
